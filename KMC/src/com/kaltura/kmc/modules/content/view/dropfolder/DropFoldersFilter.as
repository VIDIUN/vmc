package com.vidiun.vmc.modules.content.view.dropfolder
{
			...
			if (!_buttons) {
				...
				if (!value) {
					...
					for each (var profid:int in profids) {
						...
						for (i = 1; i < _buttons.length; i++) {
							if ((_buttons[i].data as VidiunDropFolder).id == profid) {
