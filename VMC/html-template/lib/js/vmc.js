// js file used by VMC.html (generated from index.template)
var vmc = {vars:{}};
...
vmc.functions = {
	checkForOngoingProcess : function() {
		var warning_message;
		try {
			warning_message = $("#VMC")[0].hasOngoingProcess();
		}
		catch(e) {
...
vmc.preview_embed = {
	...
	doPreviewEmbed : function(id, name, description, previewOnly, is_playlist, uiconf_id, live_bitrates, has_mobile_flavors, html5_compatible) {
		...
		if(id != "multitab_playlist") {
			...
			if(vmc.vars.current_uiconf) { // set by vmc.mediator.selectContent
											// called from appstudio's "select
											// content" action
				if((is_playlist && vmc.vars.current_uiconf.is_playlist) || (!is_playlist && !vmc.vars.current_uiconf.is_playlist)) { // @todo:
																																		// minor
																																		// optimization
																																		// possible
		...
		vmc.layout.modal.open( {
			...
			'title' : id_type + ': ' + name,
			'help' : '<a class="help icon" target="_blank" href="' + vmc.vars.service_url + '/index.php/vmc/vmc2help#section118"></a>',
		...
		$("#delivery_type").change(function(){
			...
			vmc.preview_embed.doPreviewEmbed(id, name, description, previewOnly, is_playlist, uiconf_id, live_bitrates, has_mobile_flavors, html5_compatible);
		});
		$("#player_select").change(function(){
			vmc.preview_embed.doPreviewEmbed(id, name, description, previewOnly, is_playlist, this.value, live_bitrates, has_mobile_flavors, html5_compatible);
		});
	...
	buildRtmpOptions : function() {
		...
		'while Progressive Download allows buffering of the content. <a target="_blank" href="' + vmc.vars.service_url + '/index.php/vmc/vmc2help#1431">Read more</a></div><br />';
		return html;
	},
	...
	buildHTML5Option : function(entry_id, partner_id, uiconf_id, has_mobile_flavors) {
		...
		var description = '<div class="note red">This video does not have video flavors compatible with IPhone & IPad. <a target="_blank" href="' + vmc.vars.service_url + '/index.php/vmc/vmc2help#section1432">Read more</a></div>';
		if(has_mobile_flavors) {
			description = '<div class="note">If you enable the HTML5 player, the viewer device will be automatically detected.' +
			' <a target="_blank" href="' + vmc.vars.service_url + '/index.php/vmc/vmc2help#section1432">Read more</a>' +
	...
	doFlavorPreview : function(entry_id, entry_name, flavor_details) {
		...
		vmc.layout.modal.open( {
			...
			'height' : parseInt(vmc.vars.default_vdp.height) + 200,
	...
	buildVidiunEmbed : function(id, name, description, is_playlist, uiconf, html5) {
		...
		uiconf_details = (typeof uiconf == "object") ? uiconf : vmc.preview_embed.getUiconfDetails(uiconf_id,is_playlist),  // getUiconfDetails
																															// returns
																															// json
		...
		if(is_playlist && id != "multitab_playlist") {	// playlist (not
			...
			// vmc.log(uiconf_details.swf_version);
	...
	getUiconfDetails : function(uiconf_id,is_playlist) {
		...
		$("#VMC")[0].alert("getUiconfDetails error: uiconf_id "+uiconf_id+" not found in " + ((is_playlist) ? "vmc.vars.playlists_list" : "vmc.vars.players_list"));
		return false;
	},
...
vmc.client = {
	...
	createShortURL : function(url) {
		...
		var data = {
			...
			"shortLink:systemName"	: "VMC-PREVIEW", // Unique name for
														// filtering
...
function openPlayer(emptystring, width, height, uiconf_id, previewOnly) { 
	if (previewOnly==true) $("#VMC")[0].alert('previewOnly from studio');
	vmc.preview_embed.doPreviewEmbed("multitab_playlist", null, null, previewOnly, true, uiconf_id); 
}
