package com.vidiun.vmc.modules.analytics.commands
{
	...
	import com.vidiun.commands.report.ReportGetBaseTotal;
	...
	public class GetBaseTotalsCommand implements ICommand, IResponder
	{
		...
		public function execute(event:CairngormEvent):void
		{
			...
			if (!_model.selectedReportData.pager) {
				_model.selectedReportData.pager = new VidiunFilterPager();
			...
			var filter:VidiunEndUserReportInputFilter = ExecuteReportHelper.createEndUserFilterFromCurrentReport(_model.getFilterForScreen((event as ReportEvent).screenType));
			...
			getBaseTotals.addEventListener(VidiunEvent.COMPLETE, result);
			getBaseTotals.addEventListener(VidiunEvent.FAILED, fault);
			_model.vc.post(getBaseTotals);
		}
